...
var SECTION_TOOLS = /* @__PURE__ */ new Map([
...
function filterPrompt(assembly, settings, registeredNames = assembly.tools.map((tool) => tool.name)) {
  const disabled = new Set(settings.disabledTools);
  const available = new Set(registeredNames.filter((name2) => !disabled.has(name2)));
  const sections = assembly.sections.filter((section) => {
    const owners = SECTION_TOOLS.get(section.name);
    if (owners) {
      return !owners.some((name2) => disabled.has(name2)) || owners.some((name2) => available.has(name2));
    }
