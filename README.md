...
关闭工具也会从后续请求中移除对应的 `tool:<工具名>` 系统提示词段落，例如 `web_search`、`web_fetch`、`write`；重新开启后恢复。Web 工具说明中对另一个已禁用 Web 工具的引用也会同步移除。Jobs、Goal、终端、会话查询和 Cordis 的共享说明在当前会话没有仍可用的相关工具时移除，部分禁用时保留共享说明；关闭 `read` 也会移除文件引用的读取提示。不按正文关键词删除其他提示词，第三方插件自定义段落、完整 persona、历史消息以及 Code/PTC SDK 不在此过滤范围内。
